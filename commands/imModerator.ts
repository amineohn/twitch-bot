...
const imModerator: ICommand = {
  ...
  execute: (
    ...
    if (permission !== Permission.MOD) {
      logger
        .warn(
          `* ${userState.username} tried to use the command "imModerator" but doesn't have the permission to do so!`
        )
        .then((r) =>
          client.say(
            args[0],
            `@${userState.username} you don't have the permission to use this command!`
          )
        );
      return;
    }
    ...
    client
      .say(args[0], moderator)
      .then((r) => logger.warn(`* replied with "${moderator}"`));
  },
};
