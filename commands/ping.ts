...
const ping: ICommand = {
    ...
    execute: (client: Client | null, args: string[], permission: Permission,  logger: Loggers) => {
        ...
        if(permission !== Permission.SUBSCRIBER) {
            logger.warn(`* ${args[1]} tried to use the command "ping" but doesn't have the permission to do so!`)
                .then(r => client?.say(args[0], `@${args[1]} you don't have the permission to use this command!`));
            return;
        }
        if (client === null)
            return;
