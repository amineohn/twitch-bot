  ...
  public async handle(message: string, channel: string, tags: ChatUserstate) {
    ...
    if (cmd.permission === Permission.MOD && !tags.mod) {
      await this.logger.warn(
        `* User ${tags.username} tried to use a mod command!`
      );
      return;
    }
    if (cmd.permission === Permission.SUBSCRIBER && !tags.subscriber) {
      await this.logger.warn(
        `* User ${tags.username} tried to use a subscriber command!`
      );
      return;
    }
    if (
      cmd.permission === Permission.BROADCASTER &&
      tags.username !== channel.slice(1)
    ) {
      await this.logger.warn(
        `* User ${tags.username} tried to use a broadcaster command!`
      );
      return;
    }
